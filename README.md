...
En este ejercicio se implementó la capacidad de enviar apuestas en lotes (batchs) desde los clientes al servidor, optimizando el flujo de transmisión y reduciendo overhead de conexión.

### Protocolo de Mensajería

- **Formato de batch**:  
  Cada lote se envía como un frame con encabezado de 4 bytes (big-endian) seguido de un cuerpo textual.  
  ```
  BATCH|<count>\n
  BET|<dni>|<numero>|<nombre>|<apellido>|<nacimiento>|<agencia_id>\n
  BET|<dni>|...
...
- **Respuesta del servidor**:  
  - Éxito: `ACKB|OK|<count>`  
  - Falla: `ACKB|FAIL|<code>|<reason>`  

El framing asegura que no ocurran **short reads/writes** y mantiene la coherencia de los mensajes.
...
- **Éxito**:
  ...
  action: receive_ack | result: success | type: ack_batch | client_id: 2 | count: 10
  ```

- **Error**:
  ```
  action: apuesta_recibida | result: fail | cantidad: 10 | error: STORE_FAILED
  action: receive_ack | result: fail | step: nack | client_id: 2 | error: STORE_FAILED: invalid bet
  ```

### Decisiones Tomadas

- Se definió un **protocolo textual** con encabezado `BATCH|N` seguido de líneas `BET|...`, lo que facilita el debug y mantiene consistencia con el protocolo individual.  
...
## Descripción
En este ejercicio se incorporó la etapa final del flujo de apuestas: la **notificación de finalización** por parte de los clientes y la **consulta de ganadores**.  
Cada cliente, una vez enviadas todas sus apuestas, notifica al servidor mediante un mensaje especial.  
El servidor, al recibir todas las notificaciones de las agencias, ejecuta el sorteo evaluando cada apuesta y permitiendo luego que los clientes consulten por sus ganadores.

## Protocolo de Mensajería
Se definieron dos nuevos tipos de mensajes en el protocolo textual:

1. **Notificación de finalización (cliente → servidor):**  
   ```
...
2. **Consulta de ganadores (cliente → servidor):**  
   ```
   WINQ|<agency_id>
   ```
   - Si el sorteo aún no se realizó:
     ```
     WRES|FAIL|NOT_READY|sorteo no realizado
     ```
...
## Logs de Ejecución
- **Cliente al notificar finalización exitosa:**
  ```
  action: notify_done | result: success | client_id: <ID>
  ```

- **Servidor al recibir todas las notificaciones y ejecutar el sorteo:**
  ```
  action: sorteo | result: success
  ```

- **Cliente al consultar ganadores:**
  ```
  action: consulta_ganadores | result: success | cant_ganadores: <CANT>
  ```

- **Servidor cuando aún no está listo para responder:**
  ```
  action: consulta_ganadores | result: fail | error: NOT_READY: sorteo no realizado
  ```

## Decisiones Tomadas
- Se agregó la variable de entorno `REQUIRED_AGENCIES` para parametrizar cuántas agencias deben notificar antes de ejecutar el sorteo (por defecto, 5).  
- Se mantuvo un registro en memoria (`_finished`) para controlar qué agencias ya notificaron.  
- El servidor almacena los ganadores en `_winners_by_agency` para responder consultas posteriores.  
- Se priorizó no realizar **broadcast**, sino responder únicamente a la agencia solicitante, cumpliendo con la consigna.
